...
// Local storage only for token + user cache (stats are server-driven)
const S = {
  get: (k, d = null) => {
    try {
...
/* Auth tabs */
.auth-tab{
  flex:1;padding:11px;text-align:center;cursor:pointer;font-size:14px;
  font-weight:600;border-radius:10px;transition:all 0.25s;color:var(--text2);
}
.auth-tab.on{background:linear-gradient(135deg,var(--cyan),var(--blue));color:#000;}
.auth-err{color:var(--red);font-size:13px;text-align:center;padding:6px 0;animation:fadeInUp 0.3s}

/* Loading spinner */
.spin{
  width:22px;height:22px;border:3px solid rgba(255,255,255,0.15);
  border-top-color:var(--cyan);border-radius:50%;
  animation:rotate 0.7s linear infinite;display:inline-block;
}
...
function MatchingTest({ words, onFinish, onXP }) {
    ...
    if (side === "left") {
      ...
      if (selRight) {
        ...
        if (ok) {
          ...
          const newMatched = [...matchedIds, item.wid];
          setBatchState(bs => ({ ...bs, matched: newMatched }));
          setSelLeft(null); setSelRight(null);
          if (newMatched.length >= left.length) {
  ...
  }, [selLeft, selRight, matchedIds, left, allResults, batchIdx, totalXP]);

  const nextBatch = (newRes) => {
    setAllResults(newRes);
    if (batchIdx + 1 >= batches.length) {
      onFinish({ results: newRes, score: newRes.filter(r => r.ok).length, xp: newRes.filter(r => r.ok).length * 5 });
    } else {
      setBatchIdx(b => b + 1);
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchRanking = async () => {
      ...
      try {
        ...
        if (!cancelled) setRanking(Array.isArray(data) ? data : (data.ranking || []));
      } catch (err) {
        if (!cancelled) setError(err.message || "Reytingni yuklashda xato");
      } finally {
    ...
    <div>
      ...
      {loading && (
        <div className="gc tc" style={{ padding: "50px 0" }}>
          ...
          <div className="muted sm">Yuklanmoqda...</div>
        </div>
      )}

      {error && !loading && (
        <div className="gc tc" style={{ padding: "32px 0", borderColor: "rgba(255,56,56,0.3)" }}>
          <div style={{ fontSize: 40, marginBottom: 10 }}>⚠️</div>
          <div style={{ color: "var(--red)", fontWeight: 600, marginBottom: 6 }}>Xato yuz berdi</div>
          <div className="muted sm">{error}</div>
        </div>
      )}
      ...
      {!loading && !error && ranking.map((u, i) => (
        <div key={u._id || u.id || u.name} className={`rk-item${u.name === user?.name ? " me" : ""}`}
          ...
          <div className="rk-xp">{u.xp || u.totalXp || 0} XP</div>
        </div>
      ))}
    </div>
  );
...
// ════════════════════════════════════════
// PROFILE PAGE — server-driven stats update
// ════════════════════════════════════════
...
function ProfilePage({ user, stats, onUpdateStats, onUpdateUser, onLogout }) {
  ...
  const handleSaveName = async () => {
    ...
    try {
      const updated = await apiFetch("/me", {
        method: "PATCH",
        body: JSON.stringify({ name: en.trim() }),
      });
      onUpdateUser({ ...user, name: updated.name || en.trim() });
      setEd(false);
    } catch (err) {
      setSaveErr(err.message || "Saqlashda xato");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleClearStats = async () => {
    try {
      await apiFetch("/me/stats/reset", { method: "POST" });
    } catch {
      // fail silently — update local anyway
    }
          ...
          {ed ? (
            <div className="f fc gap10">
              ...
              <div className="f gap8">
                <button className="btn bp f1" disabled={saving || !en.trim()} onClick={handleSaveName}>
                  {saving ? <span className="spin" /> : "Saqlash"}
                </button>
                <button className="btn bs f1" onClick={() => { setEd(false); setSaveErr(null); }}>Bekor</button>
              </div>
            </div>
          ) : (
            <>
      ...
      {sh && (
        <div className="ov" onClick={() => setSh(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="f gap12">
              <button className="btn bd f1" onClick={handleClearStats}>Ha, tozalayman</button>
...
// ════════════════════════════════════════
// AUTH MODAL — Login + Register via API
// ════════════════════════════════════════
...
function AuthModal({ onSuccess }) {
  ...
  const reset = () => { setName(""); setUsername(""); setPassword(""); setError(null); };

  const handleLogin = async () => {
    if (!username.trim() || !password.trim()) { setError("Login va parol kiriting"); return; }
    ...
    try {
      ...
      S.set("kc_user", data.user);
      onSuccess(data.user, data.stats || {});
    } catch (err) {
      setError(err.message || "Login xatosi. Tekshirib ko'ring.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRegister = async () => {
    if (!name.trim() || !username.trim() || !password.trim()) { setError("Barcha maydonlarni to'ldiring"); return; }
    if (password.length < 4) { setError("Parol kamida 4 ta belgi bo'lishi kerak"); return; }
    setLoading(true); setError(null);
    try {
    ...
    <div className="ov">
      <div className="modal">
        <div className="tc mb20">
          ...
          }}>Korean Center AI</div>
        </div>

        {/* Tab switcher */}
        <div className="f gap6 mb20" style={{ background: "var(--glass)", borderRadius: 13, padding: 4 }}>
          <div className={`auth-tab${tab === "login" ? " on" : ""}`} onClick={() => { setTab("login"); reset(); }}>
            Kirish
          </div>
          <div className={`auth-tab${tab === "register" ? " on" : ""}`} onClick={() => { setTab("register"); reset(); }}>
            Ro'yxatdan o'tish
          </div>
        </div>
        ...
        <div className="f fc gap12">
          {tab === "register" && (
            <input className="inp" value={name} onChange={e => setName(e.target.value)}
              placeholder="To'liq ismingiz *" onKeyDown={handleKey} autoFocus />
          )}
          <input className="inp" value={username} onChange={e => setUsername(e.target.value)}
            placeholder="Login (username) *" onKeyDown={handleKey}
            autoFocus={tab === "login"} autoComplete="username" />
          <input className="inp" type="password" value={password} onChange={e => setPassword(e.target.value)}
            placeholder="Parol *" onKeyDown={handleKey} autoComplete={tab === "login" ? "current-password" : "new-password"} />

          {error && <div className="auth-err">⚠️ {error}</div>}

          <button className="btn bp w100" style={{ padding: 15, marginTop: 4 }}
            onClick={tab === "login" ? handleLogin : handleRegister}
            disabled={loading}>
            {loading
              ? <><span className="spin" style={{ width: 18, height: 18 }} /> Kuting...</>
...
export default function App() {
  ...
  const [authReady, setAuthReady] = useState(false); // true once /me check is done
  ...
  const addNotif = useCallback((ic, tx) => {
    const id = Date.now() + Math.random();
    setNotifs(n => [...n, { id, ic, tx }]);
  }, []);

  // Push XP delta to server (fire-and-forget; local state is the source of truth for UI)
  const pushStatsToServer = useCallback(async (ns) => {
    try {
      await apiFetch("/me/stats", {
        method: "PATCH",
  ...
  const updateStats = useCallback((ns) => {
    setStats(ns);
    S.set("kc_stats", ns);
    pushStatsToServer(ns);
  }, [pushStatsToServer]);
  ...
  const addXP = useCallback(() => {}, []); // XP handled inside updateStats

  const handleAuthSuccess = useCallback((u, st) => {
    setUser(u);
    setStats(st);
    setShowAuth(false);
  ...
  const goPage = useCallback((p) => { setPage(p); setSidebarOpen(false); }, []);
  ...
  return (
    <>
      ...
      {/* Auth modal — shown when no valid session */}
      {authReady && showAuth && (
      ...
      {!authReady && (
        <div style={{
          ...
          flexDirection: "column", gap: 16, zIndex: 500,
        }}>
          <div className="spin" style={{ width: 40, height: 40 }} />
          <div className="muted sm">Tekshirilmoqda...</div>
        </div>
      )}
