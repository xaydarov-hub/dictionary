...
// ── Demo ma'lumotlar (haqiqiy data import bo'lmaganda) ──────────
...
const S = {
  get: (k, d = null) => {
    try { const v = localStorage.getItem(k); return v ? JSON.parse(v) : (k in memStore ? memStore[k] : d); }
    catch { return k in memStore ? memStore[k] : d; }
  },
  set: (k, v) => {
...
function AuthPage({ onLogin }) {
  ...
  const getUsers = () => S.get("kc_users", {});
  const saveUsers = (u) => S.set("kc_users", u);

  const doLogin = () => {
    if (!user.trim() || !pass.trim()) { setErr("Login va parolni kiriting"); return; }
    const users = getUsers();
    const u = users[user.trim().toLowerCase()];
    if (!u) { setErr("Foydalanuvchi topilmadi"); return; }
    if (u.pass !== pass) { setErr("Parol noto'g'ri"); return; }
    setErr("");
    onLogin(u);
  };

  const doRegister = () => {
    if (!name.trim() || !user.trim() || !pass.trim()) { setErr("Barcha maydonlarni to'ldiring"); return; }
    if (pass !== pass2) { setErr("Parollar mos kelmadi"); return; }
    if (pass.length < 4) { setErr("Parol kamida 4 ta belgi bo'lishi kerak"); return; }
    const users = getUsers();
    const key = user.trim().toLowerCase();
    if (users[key]) { setErr("Bu login allaqachon mavjud"); return; }
    const newUser = {
    ...
    saveUsers(users);
    // XP ma'lumoti
    const stats = S.get("kc_stats_" + newUser.id, {});
    S.set("kc_stats_" + newUser.id, stats);
    setErr("");
    onLogin(newUser);
  };
    ...
    <div className="ov">
      <div className="modal" style={{ maxWidth:420 }}>
        <div className="tc mb16">
          ...
          <div style={{ fontFamily:"'Orbitron'", fontSize:20, fontWeight:900, background:"linear-gradient(135deg,var(--cyan),var(--purple))", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>
            Korean Center
          </div>
          <div className="muted sm mt8">AI Vocabulary Platform</div>
        </div>

        <div className="auth-tabs">
          <div className={`auth-tab${tab==="login"?" on":""}`} onClick={()=>{setTab("login");setErr("")}}>🔑 Kirish</div>
          <div className={`auth-tab${tab==="register"?" on":""}`} onClick={()=>{setTab("register");setErr("")}}>📝 Ro'yxatdan o'tish</div>
        </div>

        <div className="f fc gap12">
          {tab === "register" && (
            <input className="inp" value={name} onChange={e=>setName(e.target.value)}
              placeholder="To'liq ismingiz *" autoFocus />
          )}
          ...
          {err && (
            <div style={{ color:"var(--red)", fontSize:13, fontWeight:600, padding:"10px 14px", background:"rgba(255,56,56,0.08)", borderRadius:10, border:"1px solid rgba(255,56,56,0.2)" }}>
              ⚠️ {err}
            </div>
          )}
          <button className="btn bp w100" style={{ padding:15, fontSize:15, marginTop:4 }}
...
function RankingPage({ user }) {
  const [selected, setSelected] = useState(null); // tanlangan foydalanuvchi

  // Barcha foydalanuvchilar statistikasini yig'ish
  const getAllRanking = () => {
    const users = S.get("kc_users", {});
    const ranking = [];
    Object.values(users).forEach(u => {
      const st = S.get("kc_stats_" + u.id, {});
      const history = S.get("kc_history_" + u.id, []);
      ranking.push({
        id: u.id,
        name: u.name,
        login: u.login,
        xp: st.xp || 0,
        testsCompleted: st.testsCompleted || 0,
        correctAnswers: st.correctAnswers || 0,
        totalAnswers: st.totalAnswers || 0,
        completedWords: st.completedWords || 0,
        history,
      });
    });
  ...
  const ranking = getAllRanking();
  const cols    = ["#ffd60a","#94a3b8","#cd7c2f"];
  const med     = ["🥇","🥈","🥉"];
  ...
  const acc = (u) => u.totalAnswers > 0 ? Math.round((u.correctAnswers/u.totalAnswers)*100) : 0;
      ...
      <div className="ov" onClick={onClose}>
        <div className="modal" style={{ maxWidth:480 }} onClick={e=>e.stopPropagation()}>
          <div className="f ic jb mb20">
            <div>
              <div style={{ fontFamily:"'Orbitron'", fontSize:18, fontWeight:900, color:"var(--cyan)" }}>{u.name}</div>
              <div className="muted sm mt8">@{u.login}</div>
            </div>
            <button className="btn bs" style={{ padding:"6px 14px" }} onClick={onClose}>✕</button>
          </div>
          {/* Asosiy stats */}
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:20 }}>
            {[
              { l:"XP",     v:u.xp,              ic:"⚡" },
              { l:"Testlar",v:u.testsCompleted,   ic:"📝" },
              { l:"Aniqlik",v:acc(u)+"%",         ic:"🎯" },
            ].map((s,i)=>(
              <div key={i} className="sc" style={{ padding:"14px 12px" }}>
                <div style={{ fontSize:18 }}>{s.ic}</div>
                <div className="sc-lbl" style={{ fontSize:10 }}>{s.l}</div>
                <div className="sc-val" style={{ fontSize:18 }}>{s.v}</div>
              </div>
            ))}
          </div>
          {/* XP manbalari */}
          <div className="fw7 mb12">📊 XP manbalari:</div>
          {Object.keys(bySource).length === 0 ? (
            <div className="muted sm tc" style={{ padding:"20px 0" }}>Hali test topshirilmagan</div>
          ) : (
            Object.entries(bySource).map(([src, data])=>{
              const sl = SOURCE_LABELS[src] || { l:src, col:"var(--text2)" };
              const pct = u.xp > 0 ? Math.round((data.xp/u.xp)*100) : 0;
              return (
                <div key={src} style={{ marginBottom:10 }}>
                  <div className="f ic jb mb6">
                    <span style={{ fontSize:13, fontWeight:600, color:sl.col }}>{sl.l}</span>
                    <span style={{ fontFamily:"'JetBrains Mono'", fontSize:12, color:"var(--gold)" }}>+{data.xp} XP ({data.count} marta)</span>
                  </div>
          ...
          {u.history && u.history.length > 0 && (
            <>
              <div className="sep" />
              <div className="fw7 mb12">🕐 So'nggi faollik:</div>
              {u.history.slice(-5).reverse().map((h,i)=>{
                const sl = SOURCE_LABELS[h.source] || { l:h.source, col:"var(--text2)" };
                return (
                  <div key={i} className="rk-detail-row">
                    <span style={{ color:sl.col, fontSize:12 }}>{sl.l}</span>
                    <span className="f1 muted sm">{h.word || ""}</span>
                    <span style={{ color:"var(--gold)", fontFamily:"'JetBrains Mono'", fontSize:11 }}>+{h.xp} XP</span>
  ...
  return (
    <div>
      ...
      <p className="psub">Barcha o'quvchilar · Bosing — batafsil ko'rish</p>

      {ranking.length === 0 ? (
        <div className="gc tc" style={{ padding:"50px 0" }}>
          <div style={{ fontSize:52, opacity:0.3, marginBottom:14 }}>🏆</div>
          <div className="fw7 muted">Hali hech kim ro'yxatdan o'tmagan</div>
        </div>
      ) : (
        ranking.map((u,i)=>(
          <div key={u.id} className={`rk-item${u.id===user?.id?" me":""}`}
            ...
            <div className="f1">
              <div className="fw7 sm">{u.name}{u.id===user?.id?" (Sen)":""}</div>
              <div className="muted sm">@{u.login} · {u.testsCompleted} test · {acc(u)}% aniqlik</div>
            </div>
            <div className="f fc ic" style={{ gap:2 }}>
              <div className="rk-xp">{u.xp}</div>
              <div className="muted sm" style={{ fontSize:10 }}>XP</div>
            </div>
          </div>
        ))
      )}

      {selected && <DetailModal u={selected} onClose={()=>setSelected(null)} />}
    </div>
  );
}
